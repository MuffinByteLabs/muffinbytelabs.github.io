...
const EXTI_REGS = [
  { name:"IMR",   off:0x00, desc:"Interrupt mask. Bit per line — set to enable IRQ generation on that EXTI line." },
  { name:"EMR",   off:0x04, desc:"Event mask. Bit per line — set to enable EVENT generation (no NVIC IRQ, but wakes the CPU from sleep). Use IMR for interrupts, EMR for wake-only events." },
  { name:"RTSR",  off:0x08, desc:"Rising trigger selection. Bit per line — set to fire on rising edge." },
  { name:"FTSR",  off:0x0C, desc:"Falling trigger selection. Bit per line — set to fire on falling edge." },
  { name:"SWIER", off:0x10, desc:"Software interrupt event. Write 1 to bit X to manually fire EXTI line X (sets PR[X], requests IRQ if IMR[X]=1). Useful for testing handlers without real hardware events." },
  { name:"PR",    off:0x14, desc:"Pending register. Bit set when trigger fired. Write 1 to clear (rc_w1) — yes, you clear by writing 1, not 0. Forgetting to clear keeps the IRQ firing." },
];
const SYSCFG_REGS = [
  { name:"MEMRMP",  off:0x00, desc:"Memory remap. MEM_MODE[1:0] selects what's mapped at 0x00000000, overriding the BOOT pins after reset. 00=Main Flash, 01=System memory, 10=FSMC Bank1, 11=SRAM1. After reset this register reflects what the BOOT pins selected; software can change it later." },
  { name:"PMC",     off:0x04, desc:"Peripheral mode config. Bit 23 = MII_RMII_SEL (Ethernet PHY interface select). Set before enabling the MAC clock; only matters for Ethernet projects." },
  { name:"EXTICR1", off:0x08, desc:"EXTI line 0–3 source selection (which port drives each line). 4 bits per line: 0000=PA, 0001=PB, 0010=PC, etc." },
...
const SYSTICK_REGS = [
  ...
  { name:"STRVR",  off:0x04, desc:"Reload Value. 24-bit. Counter reloads to this on reaching 0. NOTE (Generic User Guide §4.4.2): write (desired_ticks - 1) — for 100 cycles, write 99. Counter counts through 0 inclusively." },
...
const NVIC_REGS = [
  ...
  { name:"STIR",   off:0xF00, desc:"Software Trigger Interrupt. INTID field is 9 bits [8:0]; ARMv7-M architectural range is 0–239 (per Generic User Guide Table 4-10), not 0–511. STM32F4 implements 82 IRQs (0–81). Write the IRQ number to trigger it. Write-only." },
];
...
const DATA = {
  root: [
    ...
    { id:"extram", name:"External RAM", addr:"0x60000000–0x9FFFFFFF", size:"1 GB", color:"gray", bus:"System bus", desc:"On STM32F4: addressable via FSMC (Flexible Static Memory Controller) as banks 1–4 for off-chip NOR/PSRAM/NAND/PCCARD. Discovery board: no external memory chips connected, so no usable storage here." },
    { id:"extdev", name:"External Device", addr:"0xA0000000–0xDFFFFFFF", size:"1 GB", color:"gray", bus:"System bus", xn:true, desc:"Starts with FSMC control registers (AHB3) at 0xA0000000. Rest unused on Discovery board.", children:"extdev" },
    { id:"ppb", name:"Private Peripheral Bus", addr:"0xE0000000–0xE00FFFFF", size:"1 MB", color:"ppb_hdr", bus:"PPB", xn:true, desc:"ARM core peripherals: NVIC, SysTick, SCB (operational); ITM, DWT, FPB, TPIU, ETM, Core Debug, ROM Table (debug/trace).", children:"ppb" },
    { id:"vendor", name:"Vendor-specific", addr:"0xE0100000–0xFFFFFFFF", size:"511 MB", color:"gray", bus:"System bus", desc:"Not used" },
  ],
  code: [
    { id:"code.alias", name:"Boot Alias", addr:"0x00000000–0x000FFFFF", size:"1 MB", color:"flash", desc:"Aliased region — what's mapped here is selected by the BOOT[1:0] pins at reset (RM0090 §2.4): BOOT0=0 → main Flash; BOOT1=0,BOOT0=1 → System memory (factory bootloader); BOOT1=1,BOOT0=1 → SRAM1. CPU fetches the initial SP from 0x00000000 and the Reset Handler address from 0x00000004 here on every reset.", children:"flash" },
    { id:"code.gap1", name:"Reserved", addr:"0x00100000–0x07FFFFFF", size:"~127 MB", color:"dim" },
  ...
  sram: [
    { id:"sram.bb", name:"Bit-band Region", addr:"0x20000000–0x200FFFFF", size:"1 MB", color:"sram_i", desc:"Cortex-M4 architectural bit-band window — 1 MB. On the F407, only the first 128 KB is real RAM (SRAM1=112 KB + SRAM2=16 KB at 0x20000000–0x2001FFFF). The rest of the 1 MB window is reserved/unused. The other 64 KB that makes up the chip's '192 KB total' is CCM, which sits separately at 0x10000000 and is NOT in this bit-band region.", children:"sram_layout" },
  ...
  extdev: [
    { id:"extdev.ahb3", name:"AHB3 — FSMC Control Registers", addr:"0xA0000000–0xA0000FFF", size:"4 KB", color:"gray", bus:"System bus", desc:"Flexible Static Memory Controller registers. Configures FSMC banks 1–4 (which live in the External RAM region 0x60000000–0x9FFFFFFF). ST classifies this as the AHB3 bus even though the address sits in ARM's External Device region. Nothing connected on Discovery board." },
  ],
  ahb1: [
    { id:"ahb1.gpioa", name:"GPIOA", addr:"0x40020000–0x400203FF", size:"1 KB", color:"ahb1", baseAddr:0x40020000, registers:GPIO_REGS, clock:"AHB1ENR bit 0", disc:"Discovery board: PA0=B1 user button. PA4=I2S3 LRCK to CS43L22 / DAC1_OUT. PA5/6/7=SPI1 to MEMS accelerometer (CS on PE3). PA9–PA12=USB OTG FS (micro-AB connector). PA13/PA14=SWD debug pins — REPURPOSING THESE BREAKS ST-LINK CONNECTION (you'll need a power-cycle reset to recover).", desc:"Port A. 16 pins." },
    { id:"ahb1.gpiob", name:"GPIOB", addr:"0x40020400–0x400207FF", size:"1 KB", color:"ahb1", baseAddr:0x40020400, registers:GPIO_REGS, clock:"AHB1ENR bit 1", disc:"Discovery board: PB6 (I2C1 SCL) and PB9 (I2C1 SDA) → CS43L22 audio codec control. PB10=clock input to onboard MEMS microphone (driving PB10 with a non-clock signal corrupts mic data, harmless if you're not using it).", desc:"Port B. 16 pins." },
    { id:"ahb1.gpioc", name:"GPIOC", addr:"0x40020800–0x40020BFF", size:"1 KB", color:"ahb1", baseAddr:0x40020800, registers:GPIO_REGS, clock:"AHB1ENR bit 2", disc:"Discovery board: PC3=MEMS microphone PDM data input. PC7=I2S3 MCK, PC10=I2S3 SCK, PC12=I2S3 SD — audio data to CS43L22 codec. PC14/PC15=external 32.768 kHz crystal (LSE) for RTC — don't reuse as GPIO if you need accurate RTC.", desc:"Port C. 16 pins." },
    { id:"ahb1.gpiod", name:"GPIOD", addr:"0x40020C00–0x40020FFF", size:"1 KB", color:"ahb1_lt", baseAddr:0x40020C00, registers:GPIO_REGS, clock:"AHB1ENR bit 3", disc:"Discovery board: PD4=CS43L22 RESET (active low — must drive high to bring codec out of reset). PD5=USB OTG FS power switch enable. PD12=green LED (LD4), PD13=orange (LD3), PD14=red (LD5), PD15=blue (LD6).", desc:"Port D. Discovery board LEDs." },
    { id:"ahb1.gpioe", name:"GPIOE", addr:"0x40021000–0x400213FF", size:"1 KB", color:"ahb1", baseAddr:0x40021000, registers:GPIO_REGS, clock:"AHB1ENR bit 4", disc:"Discovery board: PE0=accelerometer INT1, PE1=accelerometer INT2, PE3=accelerometer chip select (CS_I2C/SPI — held low during SPI transfers).", desc:"Port E. 16 pins." },
    ...
    { id:"ahb1.crc", name:"CRC", addr:"0x40023000–0x400233FF", size:"1 KB", color:"ahb1", baseAddr:0x40023000, clock:"AHB1ENR bit 12", desc:"CRC calculation unit. Single 32-bit input/output register. Hardware CRC-32 (Ethernet polynomial 0x4C11DB7). Used for verifying flash integrity (EN/IEC 60335-1) or computing checksums fast." },
    ...
    { id:"ahb1.bkpsram", name:"BKPSRAM", addr:"0x40024000–0x40024FFF", size:"4 KB", color:"ahb1", baseAddr:0x40024000, clock:"AHB1ENR bit 18", desc:"Backup SRAM. Retains data when VDD is off, powered by VBAT (coin cell). Useful for crash logs or persistent state across power cycles. Write access requires DBP=1 in PWR_CR." },
  ...
  apb1: [
    ...
    { id:"apb1.rtc", name:"RTC & BKP Registers", addr:"0x40002800–0x40002BFF", size:"1 KB", color:"apb1_dk", baseAddr:0x40002800, clock:"RCC_BDCR.RTCEN (not APB1ENR)", desc:"Real-Time Clock + 20×32-bit backup registers. Backup domain — survives reset and powers from VBAT. Clocked by LSE/LSI/HSE selected via RCC_BDCR.RTCSEL. Write access requires DBP=1 in PWR_CR." },
    { id:"apb1.wwdg", name:"WWDG", addr:"0x40002C00–0x40002FFF", size:"1 KB", color:"apb1_dk", baseAddr:0x40002C00, clock:"APB1ENR bit 11", desc:"Window Watchdog. Must be refreshed within a time window — too early OR too late triggers reset. For detecting code that runs but is malfunctioning. Clocked from PCLK1 (42 MHz max)." },
    { id:"apb1.iwdg", name:"IWDG", addr:"0x40003000–0x400033FF", size:"1 KB", color:"apb1_dk", baseAddr:0x40003000, desc:"Independent Watchdog. Clocked by LSI (~32 kHz), independent of PCLK1 — keeps running even if main clock fails. No clock enable bit needed; once started, it can only be reset by a chip reset." },
    ...
    { id:"apb1.spi3", name:"SPI3 / I2S3", addr:"0x40003C00–0x40003FFF", size:"1 KB", color:"apb1_lt", baseAddr:0x40003C00, registers:SPI_REGS, clock:"APB1ENR bit 15", disc:"I2S3 → onboard CS43L22 audio codec (audio data: PC7 MCK, PC10 SCK, PC12 SD, PA4 LRCK)", desc:"Serial Peripheral Interface / Inter-IC Sound." },
    ...
    { id:"apb1.i2c1", name:"I2C1", addr:"0x40005400–0x400057FF", size:"1 KB", color:"apb1_dk", baseAddr:0x40005400, registers:I2C_REGS, clock:"APB1ENR bit 21", disc:"PB6 (SCL) / PB9 (SDA) → onboard CS43L22 audio codec (control)", desc:"Inter-Integrated Circuit." },
    ...
    { id:"apb1.dac", name:"DAC", addr:"0x40007400–0x400077FF", size:"1 KB", color:"apb1_lt", baseAddr:0x40007400, registers:DAC_REGS, clock:"APB1ENR bit 29", desc:"DAC. 2 channels, 12-bit. Output pins: DAC1=PA4, DAC2=PA5. Note: not connected to the CS43L22 on Discovery — that's driven by I2S3, not the on-chip DAC." },
  ],
  apb2: [
    ...
    { id:"apb2.adc1", name:"ADC1", addr:"0x40012000–0x4001204F", size:"80 B", color:"apb2_lt", baseAddr:0x40012000, addrEnd:0x4001204F, registers:ADC_REGS, clock:"APB2ENR bit 8", desc:"12-bit ADC. Master in dual/triple mode. 16 external + 3 internal channels." },
    { id:"apb2.adc2", name:"ADC2", addr:"0x40012100–0x4001214F", size:"80 B", color:"apb2_lt", baseAddr:0x40012100, addrEnd:0x4001214F, registers:ADC_REGS, clock:"APB2ENR bit 9", desc:"12-bit ADC. Slave 1. Same registers at +0x100 offset." },
    { id:"apb2.adc3", name:"ADC3", addr:"0x40012200–0x4001224F", size:"80 B", color:"apb2_lt", baseAddr:0x40012200, addrEnd:0x4001224F, registers:ADC_REGS, clock:"APB2ENR bit 10", desc:"12-bit ADC. Slave 2. Same registers at +0x200 offset." },
    { id:"apb2.adc_com", name:"ADC Common", addr:"0x40012300–0x4001230B", size:"12 B", color:"apb2_dk", baseAddr:0x40012300, addrEnd:0x4001230B, registers:ADC_COMMON_REGS, desc:"Shared: prescaler, DMA mode, multi-ADC config." },
    ...
    { id:"apb2.syscfg", name:"SYSCFG", addr:"0x40013800–0x40013BFF", size:"1 KB", color:"apb2_dk", baseAddr:0x40013800, registers:SYSCFG_REGS, clock:"APB2ENR bit 14", desc:"System Configuration controller. Memory remap (MEMRMP), Ethernet PHY select (PMC), and the EXTI line→port mux (EXTICR1–4). Must be clocked before writing EXTICRx." },
    { id:"apb2.exti", name:"EXTI", addr:"0x40013C00–0x40013FFF", size:"1 KB", color:"apb2_dk", baseAddr:0x40013C00, registers:EXTI_REGS, desc:"Extended Interrupts and Events. 23 lines total. Lines 0–15 are the GPIO external interrupts (port selected via SYSCFG_EXTICRx). Lines 16–22 are wired to internal events: 16=PVD output, 17=RTC Alarm, 18=USB OTG FS Wake-up, 19=Ethernet Wake-up, 20=USB OTG HS Wake-up, 21=RTC Tamper/TimeStamp, 22=RTC Wake-up. EXTI itself has no clock enable bit — always available. SYSCFG (APB2ENR bit 14) only needs enabling to write the EXTICRx mux registers."},
  ...
  ahb2: [
    { id:"ahb2.usbfs", name:"USB OTG FS", addr:"0x50000000–0x5003FFFF", size:"256 KB", color:"ahb2", clock:"AHB2ENR bit 7", disc:"Discovery board: PA9 (VBUS), PA10 (ID), PA11 (DM), PA12 (DP) → onboard micro-AB USB connector (CN5). LD7 (green) = VBUS present, LD8 (red) = overcurrent. PD5 enables the VBUS power switch when running as host.", desc:"USB 2.0 Full-Speed (12 Mbps). Can act as host or device." },
  ...
  ppb: [
    ...
    { id:"ppb.scs", name:"SCS (System Control Space)", addr:"0xE000E000–0xE000EFFF", size:"4 KB", color:"scs", baseAddr:0xE000E000, addrEnd:0xE000EFFF, _lookupRegs:ALL_SCS_REGS, desc:"4 KB window shared by 5 ARM blocks: NVIC, SysTick, SCB, Core Debug, and FPU control. Not one peripheral — a container.", children:"scs" },
    ...
    { id:"ppb.etm", name:"ETM", addr:"0xE0041000–0xE0041FFF", size:"4 KB", color:"ppb", baseAddr:0xE0041000, addrEnd:0xE0041FFF, desc:"Embedded Trace Macrocell. Streams compressed instruction-flow trace to a Trace Port Analyser via TPIU. Optional — present on Cortex-M4 but rarely used without an external trace probe." },
    { id:"ppb.gap3", name:"Reserved", addr:"0xE0042000–0xE00FEFFF", color:"dim" },
    { id:"ppb.romtable", name:"ROM Table", addr:"0xE00FF000–0xE00FFFFF", size:"4 KB", color:"ppb", baseAddr:0xE00FF000, addrEnd:0xE00FFFFF, desc:"CoreSight ROM table — debugger reads this to discover which components are present (SCS, DWT, FPB, ITM, TPIU, ETM). Read-only. ARM TRM §8.1.4." },
  ],
  scs: [
    ...
    { id:"scs.nvic", name:"NVIC", addr:"0xE000E004, 0xE000E100–0xE000E4EF, 0xE000EF00", size:"scattered", color:"scs", baseAddr:0xE000E000, _noLookup:true, registers:NVIC_REGS, desc:"Nested Vectored Interrupt Controller. Three non-contiguous sub-ranges (per ARM Generic User Guide Table 4-2): ICTR at 0xE000E004; ISER/ICER/ISPR/ICPR/IABR/IPR span 0xE000E100–0xE000E4EF; STIR at 0xE000EF00. Enable, disable, pend, prioritize all 82 IRQs. On the block diagram: the NVIC box (tightly coupled to processor core)." },
    ...
    { id:"scs.dcb", name:"Core Debug", addr:"0xE000EDFC", size:"4 B", color:"scs", baseAddr:0xE000E000, _noLookup:true, registers:DCB_REGS, desc:"ARM-official term: 'Core Debug' (RM0090 §38.10, ARM TRM §8.1.6). Currently shown: DEMCR only. Full Core Debug also includes DFSR (0xE000ED30), DHCSR (0xE000EDF0), DCRSR (0xE000EDF4), DCRDR (0xE000EDF8). DEMCR bit 24 (TRCENA) is the master switch for ITM/DWT/TPIU. On the block diagram: invisible — debug-only infrastructure." },
...
const NOTES = {
  ...
  scs:"System Control Space — 4 KB shared by 5 ARM blocks: NVIC, SysTick, SCB, Core Debug, FPU control. These are NOT separate peripherals with separate 1 KB address ranges — they are logical groups of registers interleaved within one 4 KB window (per ARM Generic User Guide Table 4-1).",
};
